...
| **T-7** | `customCommand` becomes an injection vector | The runner intentionally executes `customCommand` via `sh -c` — that's the documented contract. Defence is upstream: the relayer's submission handler always passes `null` today, and the executor's `validateCustomCommand` rejects NULs / C0 control bytes / DEL / C1 control bytes (0x7F-0x9F) / >4 KB inputs as a guard against accidental misuse. **`customCommand` MUST come from a trusted source (bounty-creator UI), never from PR-supplied data (e.g. `.ghbounty.yml` in the repo).** | Accepted: depends on caller discipline. If/when a future feature exposes `customCommand` to user input, that feature is broken by design and must thread its own validation. | ✓ comment in `types.ts` + validator |
